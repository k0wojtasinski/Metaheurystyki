...
class Problem:
    ...
    def from_json(cls, file_path: str):
        ...
        logger.info(f"Loading input from json file (file_name: {file_path}, content: {content})")
        return results
    ...
    def export_correct_solutions_to_json(self, file_path):
        ...
        solutions = [solution.data for solution in self.solutions if solution.is_correct()]
        self.export_solutions_to_json(solutions, file_path)

    def export_incorrect_solutions_to_json(self, file_path):
        """ method to export all incorrect solutions to JSON file """
        solutions = [solution.data for solution in self.solutions if not solution.is_correct()]
        self.export_solutions_to_json(solutions, file_path)
        ...
        for i in range(1, len(self.set) + 1):
            ...
            for combination in itertools.combinations(self.set, i):
                ...
                if solution.is_correct():
                    logger.info(f"Found solution (time={solution.time}, tries={solution.tries})")
                    return solution

        end_time = time.time() - start_time
        logger.warn(f"Solution cannot be found (time={end_time}, tries={len(self.solutions)})")
